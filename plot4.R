# Assigment 1. Plot 4 . By Isaias Sanchez Cortina (isanchez@doctor.upv.es)
...
############## 
# plot4.png : time series in 4 different plots.
#             x axes shows "Thu  Fri Sat" labels
##############


# Computations to properly set the X-axis as "Thu Fri Sat"
...
# Open PNG output device with the required sized
...
plot (x$Minute, x$Global_active_power, ylab="Global Active Power", xlab="", type="l",axes=F,col="black");
box() ; axis(2); # Draw the missing box and Add Y-axis:
axis(1, at = mytickmarks , labels = mylabels ) #  Draw a customized X-axis with weekdays as labels. 

# Subplot 2:
# 2nd  : Plot the series Voltage vs. ellapsed time in minutes (xlabel="datetime")
plot (x$Minute, x$Voltage, ylab="Voltage", xlab="datetime", type="l",axes=F,col="black");
box() ; axis(2); # Draw the missing box and Add Y-axis:
axis(1, at = mytickmarks , labels = mylabels ) #  Draw a customized X-axis with weekdays as labels. 

# Subplot 3:
# 2nd  : Plot the three series vs. ellapsed time in minutes 
# NOTE: This subplot is similar to plot3.png, but for the legend surrounding box.
plot (x$Minute, x$Sub_metering_1, ylab="Energy sub metering", xlab="", type="l",axes=F,col="black");
lines(x$Minute, x$Sub_metering_2,col="red");
lines(x$Minute, x$Sub_metering_3,col="blue");
legend("topright",names(x)[7:9],lty=1,col=c("black","red","blue"), bty="n")
box() ; axis(2); # Draw the missing box and Add Y-axis:
axis(1, at = mytickmarks , labels = mylabels ) #  Draw a customized X-axis with weekdays as labels. 

# Subplot 4:
# 2nd  : Plot the series Global_reactive_power vs. ellapsed time in minutes 
# Plot with lines and w/o title nor xlabel. Neither the axes and box will be shown
#   (axes=F) to allow customization of the x-bottom axes as required in the assigment.
plot (x$Minute, x$Global_reactive_power, ylab="Global_reactive_power", xlab="datetime", type="l",axes=F,col="black");
box() ; axis(2); # Draw the missing box and Add Y-axis:
axis(1, at = mytickmarks , labels = mylabels ) #  Draw a customized X-axis with weekdays as labels. 
